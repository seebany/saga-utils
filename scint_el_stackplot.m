...
in_path3 = ['/data1/from_usb2', sep, year, sep, doy, sep, rcvr_name, sep];
in_path4 = ['/data1/from_usb3', sep, year, sep, doy, sep, rcvr_name, sep];
...
elseif ~isempty(flag2) && any([flag2.bytes])
    in_path = in_path2;
elseif ~isempty(flag3) && any([flag3.bytes])
    in_path = in_path3;
elseif ~isempty(flag4) && any([flag4.bytes])
    in_path = in_path4;
else
    disp(['Unable to find any binaries for ', rcvr_name, ' on the server, please download manually']);
    in_path = [];
