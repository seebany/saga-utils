...
% %the log files
% folder_name = '2012_11_12/2012_11_12_LR2s';
...
%             + UTC_time(:,6); 
        ...
        %Addded 5/26/2015, Yang Su
        obstime = (datenum(UTC_time)-init_time)*24*3600;
%         keyboard;
        ...
        if(isempty(diff_ind)==0)
            disp(['Time discontinuities found for PRN ',num2str(KK)]);
...
%                 plot(diff(diff(rawphase)))
                if isempty(indphtemp) ~= 1,
                    disp(['Phase discontinuities found for PRN ',num2str(KK)]);
...
%                     close
            ...
            %save filtered data
            data_PRN = [obstime;piqpowdata;piqphdata;pcdata];
            strprn = strcat('FilteredData_PRN',num2str(kk),...
                datestr(tstt, '_HHMMUT'), '_zoom', num2str(zcounter), '.mat');
            filename = strcat(FiltDatadir,strprn);
            save(filename,'data_PRN');
            strprn = strcat('FilteredData_PRN',num2str(kk),...
                datestr(tstt, '_HHMMUT'), '_zoom', num2str(zcounter), '.txt');
            filename = strcat(FiltDatadir,strprn);
...
%             subplot(2,1,2)
%             set(gca,'FontSize',fontsz)
%             plot(obstime-3600,piqphdata,'g-','MarkerSize',marksz);
%             grid on
%             xstring = strcat({'Time [s] after '},num2str(hour),...
%                 ':00 UT on: (mm/dd/yy)',num2str(month),'/'...
%                 ,num2str(day),'/',num2str(year));
...
% command = strcat('mkdir',{' '},folder_name,'plots_',signal,sep,...
%     'M2HighRate_S4_SPhi',sep);
% system(cell2mat(command));
% outdir = strcat(folder_name,'plots_',signal,sep,'M2HighRate_S4_SPhi',sep);
...
for kk = KK;%1:1:32,%14;%
    ...
    if(isempty(svdata)~=1),
        ...
        %Commented by Yang Su 11/3/2015
%         UTC_init = UTC_time(1,:);
%         year = UTC_init(1);
%         month = UTC_init(2);
%         day = UTC_init(3);
...
%                         disp(obstime(zm_end))
                        ...
                        if Tdata >=60,
                            ...
                            [S4outphd, SPHoutphd] = ...
                                slidingS4SP_revised(fiqpowzphd,fiqphzphd,FL);
...
%                         end % only consider greater than 60 seconds long data
                    ...
                    if Tdata >=60,
                        ...
                        %Number of samples in data
                        FL = floor(Tfilt/Tdata*DataLength);
                        [S4out, SPHout] = slidingS4SP_revised(fiqpowz,fiqphz,FL);
...
%                     end % only consider greater than 60 seconds long data               
            ...
            %find discontinuities in phase data
            rawphase = piqphdata;
            indphtemp = find( abs(diff(diff(rawphase))) > 60);
            fignum = 0;
            if isempty(indphtemp) ~= 1,
                disp(['Phase discontinuities found for PRN ',num2str(KK)]);
                indph = indphtemp(2:2:end);
...
%                     end % only consider greater than 60 seconds long data
                ...
                if Tdata >=60,
                    ...
                    %Number of samples in data
                    FL = floor(Tfilt/Tdata*DataLength);
                    [S4out, SPHout] = slidingS4SP_revised(piqpowdata,piqphdata,FL);
...
%                 end % consider only longer than 60 s data
    ...
    %save filtered data:
    data_PRN = [AOBSTIME',S4DATA',SPHDATA'];
    
    zrowsp = find(data_PRN(:,3)==0);
    for zrows4 = find(data_PRN(:,2)==0);
        data_PRN(zrows4,2) = data_PRN(zrows4,2)*NaN;
        data_PRN(zrows4,3) = data_PRN(zrows4,3)*NaN;
    end
    data_PRN = data_PRN';
    strprn = strcat('HR_Scintdata_PRN',num2str(kk),...
        datestr(tstt, '_HHMMUT'), '_zoom', num2str(zcounter), '.mat');
    filename = strcat(FiltDatadir,strprn);
    save(filename,'data_PRN');
    strprn = strcat('HR_Scintdata_PRN',num2str(kk),...
...
% Create necessary directories
command = strcat('mkdir',{' '},op_path,'../plots_',signal,sep,...
    'M2HighRate_Common_times',sep);
system(cell2mat(command));
outdir = strcat(op_path,'../plots_',signal,sep,'M2HighRate_Common_times',sep);
